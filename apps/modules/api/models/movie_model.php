...
class Movie_model extends ADODB_model {
	...
	public function rewiteData(&$data){
		if(isset($data['cover'])){
			...
			for($i=0,$j=count($data);$i<$j;$i++){
				...
				if(isset($data[$i]['favorite_id'])&&is_null($data[$i]['favorite_id'])){
					$data[$i]['favorite_id'] = 0;
				}
				if(isset($data[$i]['is_new'])&&is_null($data[$i]['is_new'])){
					$data[$i]['is_new'] = 'NO';
				}
			}
	...
	public function getMovieEpisode($movie_id,$episode_id="",$member_id=""){

		if(is_numeric($member_id)){
			$isWatched = ",(SELECT mh.history_id FROM ".$this->table('movie_history','mh')." WHERE mh.user_id = ".$member_id." AND mh.movie_id = e.movie_id AND mh.episode_id = e.episode_id) AS isWatched ";
		}else{
			$isWatched = ",(SELECT 0) AS isWatched ";
		}
		
		$sql = "SELECT e.* ".$isWatched."  
				FROM ".$this->table('episode','e')."  
				WHERE e.movie_id = '".$movie_id."' 
				".(empty($episode_id)?"":" AND e.episode_id = '".$episode_id."' ")."
				AND e.status = 'ACTIVE'
	...
	public function updateView($movie_id){
		$sql = "update " . $this->table('movie') . " set view = view+1 where movie_id = $movie_id";
