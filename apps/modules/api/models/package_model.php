...
class Package_model extends ADODB_model {
	...
	public function rewiteData(&$data){
		if(isset($data['banner'])){
			...
			$data['banner_ios'] = str_replace(array('.png','.jpg'),array('_ios.png','_ios.jpg'),$data['banner']);
			
		}else{
			for($i=0,$j=count($data);$i<$j;$i++){
				$data[$i]['banner'] = static_url($data[$i]['banner']);
				$data[$i]['banner_ios'] = str_replace(array('.png','.jpg'),array('_ios.png','_ios.jpg'),$data[$i]['banner']);
			}
		}
	}
