...
class Member extends API_Controller{
	...
	public function history_post(){
		...
		$episode_id = $this->post('episode_id');
		$last_time = $this->post('last_time');
		...
		if($this->mMember->setMemberHistory($member_id,$movie_id,$episode_id,$last_time)){
			$this->success(array());
		}else{
	...
	public function favorite_delete($favorite_id=""){
	...
	public function package_get(){
		...
		if(!is_numeric($member_id)){
			...
			for($i=0,$j=count($package['items']);$i<$j;$i++){

				$package['items'][$i]['banner']  = static_url($package['items'][$i]['banner']);
			}
